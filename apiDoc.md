...
| 得到温故知新和双周测的所有历史刷题记录 | /getTable_ai || 数组<br>数组元素:对象<br>对象属性：对应刷题名称(string)、完成时间(string)、得分(number)、本条记录编号(number或string) | `"tableList":[{"name","date","score","id"},...]` | 对应刷题名称中，若为双周测则表明“xx周-xx周双周测”，若为温故知新则表明“Level xx 温故知新练习”<br>得分单位：0-100 |
| 获取用户当前等级 | /historyInitial || 等级(number) | `"level"` | 可以与其它页面获取等级的接口整合成一个<br>等级范围：1-6 |
...
| 获取本次刷题总题数、选择题数和本次刷题结果在数据库中的编号 | /ini || 总题数(number)<br>选择题题数(number)<br>本次刷题结果的编号(number或string) | `"total","opt","id"` ||
...
| 获取双周测的所有历史记录 | /getTest || 数组<br>数组元素：对象<br>对象属性：对应哪个双周(string)、时间(string)、得分(number)、本次记录编号(number或string) | `"tableList":[{"name","date","score","id"}]` ||
...
| 获取用户当前等级 | /wrongInitial || 等级(number) | `"level"` ||
***
# 页面汇总
| 页面名称 | 页面描述 | URL | 调用API |
| :----: | :----: | :----: | :---- |
| answer_sheet | 答题卡侧边栏 | / | / |
| history | 历史记录页面 | /history | /getTable_lesson<br>/getTable_ai<br>/historyInitial |
| homepage | 温故知新页面(主界面) | /home | /homepageInitial |
| include | 功能导航侧边栏 | / | / |
...
| wrong_problem | 错题集页面 | /wrong | /getwrong<br>/wrongInitial |
